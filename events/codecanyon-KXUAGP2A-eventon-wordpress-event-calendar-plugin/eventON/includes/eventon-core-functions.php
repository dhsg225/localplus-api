...
// TAXONOMIES
	...
	// @version 2.4.7 @updated 5.0
	function evo_get_term_meta($tax, $termid, $wp_option_val ='', $secondarycheck= false){
		$termmetas = !empty( $wp_option_val )? $wp_option_val : EVO()->calendar->get_tax_meta();

		if( empty($termmetas[$tax][$termid])){

			// check legacy
			if( $tax == 'event_type'){
				$legacy_meta_data = get_option( "evo_et_taxonomy_".$termid);
				if( !empty($legacy_meta_data) ) return $legacy_meta_data;
			}			
	...
	// @since 5.0
	function evo_get_term_meta_field( $tax, $termid, $field, $wp_option_val= '', $default = ''){
		$term_meta = evo_get_term_meta( $tax, $termid , $wp_option_val);
		//EVO_Debug($term_meta);

		return ( $term_meta && !empty($term_meta[ $field ]) ) ? $term_meta[ $field ] : $default;
	}
	...
	// GET activated event type count -- deprecated 4.9.2
...
// DATE & TIME
	...
	function eventon_get_unix_time($data='', $date_format='', $time_format='', $tz2= ''){
		...
		if(!empty($data['evcal_end_date']) && !empty($data['evcal_start_date'])){
			...
			if( !empty($data['evcal_start_date']) ){
				...
				if (!$datetime) {
				    error_log('Invalid date format: ' . $data['evcal_start_date'] . ' for format ' . $_wp_date_format);
				    return false;
				}
			...
			// END TIME UNIX
				...
				if (!$datetime) {
				    error_log('Invalid date format: ' . $__evo_end_date . ' for format ' . $_wp_date_format);
				    return false;
				}
				...
				if( !empty( $data['event_vir_date_x'])){
					...
					if( !empty($data['_vir_hour'])  ){
						...
						if( !empty($data['_vir_ampm']) ){
							$vir_ampm = trim( strtolower( $data['_vir_ampm'] ));
							if( $vir_ampm =='pm' && $time_hour != 12) $time_hour += 12; 
							if( $vir_ampm =='pm' && $time_hour == 12) $time_hour = 12; 
							if( $vir_ampm =='am' && $time_hour == 12) $time_hour = 0; 
						}
	...
	*/
	function eventon_return_timely_names_($type, $data, $len='full', $lang=''){
		...
		$option_name_prefix = $option_name_suffix = '';
		...
		$output = (!empty($evo_options[$_lang_variation][$option_name_prefix.$text_num . $option_name_suffix ]))? 
					$evo_options[$_lang_variation][$option_name_prefix.$text_num . $option_name_suffix ] 
					:  $_not_value;
	...
	// @updated 4.0.3
		function evo_process_to_past_events(){
		    ...
		    if (!defined('DOING_CRON') || !DOING_CRON) {
		        EVO_Debug('EventON: evo_process_to_past_events called outside cron context.');
		        return false;
		    }
		    ...
		    $delete_permanently = EVO()->cal->check_yn('evcal_delete_past_events', 'evcal_1');
			
			// No options set
		    if (!$trash && !$completed && !$delete_permanently) {
		        return false;
		    }
			...
			while($events->have_posts()): $events->the_post();
		        ...
		        if ($trash) {
		            ...
		            // Completely delete the event post
		            if ($delete_permanently) {
			            do_action('evo_before_deleting_event', $event_id);
			            ...
			            do_action('evo_after_deleting_event', $event_id);
			        }else{
...
// SUPPORT FUNCTIONS
	...
	// Generate location latLon from address @4.7.2
		function eventon_get_latlon_from_address($address){
		    ...
		    // if google maps and API key is missing
