			...
			// Do other things based on shortcode arguments
				...
				// process WPML @u 4.5,5	
					if( has_filter( 'wpml_current_language') || defined('ICL_LANGUAGE_CODE')){
						...
						if( defined('ICL_LANGUAGE_CODE')) $current_lang = ICL_LANGUAGE_CODE; // backward compatibility

						if( !empty($current_lang)){
							$lang_count = apply_filters('eventon_lang_var_count', 3); // @version 2.2.24
							for($x=1; $x <= $lang_count; $x++){
								if(!empty($args['wpml_l'.$x]) && $args['wpml_l'.$x]== $current_lang ){
									$args['lang']='L'.$x;
								}
							}
						}						
	...
	// ### Generate Events for all calendars - lists and single months
		...
		function _generate_events( $output = 'both', $wp_argument_additions = array()){
			...
			if( $number_of_months > 1 && $search_all == 'no'){			
				if( apply_filters('evo_generate_events_filter_proceed', true, $A) ){		

					$_ESA = []; // all events in the calendar IDs
					$_ECM = 0;
					$events_by_month = [];
	 ...
	 */
		...
		// for a month by default but can change to set time line with args
			public function evo_get_wp_events_array(	$wp_argument_additions='', $shortcode_args='' ){
    			...
    			apply_filters('evo_calendar_main_events_array_cahced', $event_list_array, $this, $wp_argument_additions , $SC );
				...
				// WPQUery Arguments
					...
					// query adds based on evo_settings_query_type value in settings
						if ($que = EVO()->cal->get_prop('evo_settings_query_type', 'evcal_1')) {
					        $getdate = getdate();
					        switch ($que) {
					            case 'this_year': $wp_arguments_['date_query'] = array(array('year' => $getdate["year"])); break;
					            case '12months': $wp_arguments_['date_query'] = array(array('column' => 'post_date_gmt', 'after' => '12 months ago')); break;
					            case '6months': $wp_arguments_['date_query'] = array(array('column' => 'post_date_gmt', 'after' => '6 months ago')); break;
					            case 'last_5months': $wp_arguments_['date_query'] = array(array('column' => 'post_date_gmt', 'after' => '5 months ago')); break;
					            case 'last_4months': $wp_arguments_['date_query'] = array(array('column' => 'post_date_gmt', 'after' => '4 months ago')); break;
					            case 'last_3months': $wp_arguments_['date_query'] = array(array('column' => 'post_date_gmt', 'after' => '3 months ago')); break;
					            case 'last_2months': $wp_arguments_['date_query'] = array(array('column' => 'post_date_gmt', 'after' => '2 months ago')); break;
					            case 'this_month': $wp_arguments_['date_query'] = array(array('month' => $getdate["mon"])); break;
					        }
					    }
					    if (count($meta_query) > 0) $wp_arguments_['meta_query'] = $meta_query;


					// Exclude Events @since 5.0
					    if( isset($SC['x_ids'])){
					    	$x_eids = explode(',', esc_html( $SC['x_ids']) );
					    	$wp_arguments_['post__not_in'] = $x_eids;
					    }
		...
		public function generate_event_data(
			...
			foreach($event_list_array as $event_):
					...
					// EVENT TERMS						
						if(isset($eventtop_fields['used'])){
							...
							foreach( $_valid_taxes as $key=> $nn){
								...
								$__tax_field = 'eventtype'.($key==1?'':$key);
				...
				// for ux val 4 show href value for <a>
					if($event_ux_val == '4' || $event_ux_val == '4a') $_eventInAttr['href'] = $EVENT->get_permalink();
