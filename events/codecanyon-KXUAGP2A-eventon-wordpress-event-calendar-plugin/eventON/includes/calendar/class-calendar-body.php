...
class evo_cal_body{
	...
	// calendar class generator based on shortcode arguments
		function _get_calendar_classes($__cal_classes, $args){
			...
			// filter since 4.9
			return apply_filters('evo_calendar_classes', $__cal_classes, $this) ;
		}
		...
		private function cal_main_buttons( $A  ){
			...
			if( count( $main_btns ) > 0 ){					
				...
				foreach($main_btns as $ff=>$v){
					...
					echo "<button class='cal_head_btn ".$ff.$add."' aria-label='{$ff}'>".$v. $add2 . "</button>";	
				}				
			}
					...
					echo "<div class='evo_cal_above evodfx evofx_dr_r marb5 evofx_ww'>";
						...
						foreach($above_head as $ff=>$v){
							...
							if($ff=='evo-gototoday-btn'){
								echo "<button class='cal_head_btn ".$ff."' style='' data-mo='{$focused_month_num}' data-yr='{$focused_year}' data-dy='' aria-label='{$ff}'>".$v."</button>";
							}else{
								$add =  '';
								...
								echo "<button class='cal_head_btn ".$ff.$add."' aria-label='{$ff}'>".$v.  "</button>";
							}							
						}
						...
						if( $view_switcher == 'yes' && $calendar_type != 'list'){
							...
							echo "<span class='cal_head_btn evo-viewswitcher-btn {$calendar_type} evodb' aria-label='Viewswitcher'>";
		...
		// pass calendar navigation data
			function get_cal_navigation_data( $SC ){
				...
				if(!$hide_arrows){
					...
					<button id='evcal_prev' class='evcal_arrows evcal_btn_prev evodfx evofx_jc_c evofx_ai_c' aria-label='". evo_lang('Previous Month')."'><i class='fa fa-chevron-left' aria-hidden='true'></i></button>
					<button id='evcal_next' class='evcal_arrows evo_arrow_next evcal_btn_next evodfx evofx_jc_c evofx_ai_c' aria-label='". evo_lang('Next Month')."'><i class='fa fa-chevron-right' aria-hidden='true'></i></button>
					</p>";
				}
