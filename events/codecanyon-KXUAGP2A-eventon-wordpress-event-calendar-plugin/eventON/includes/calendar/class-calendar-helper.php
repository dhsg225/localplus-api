...
class evo_cal_help {
	...
	// all available event card fields
		function get_eventcard_fields($legacy = false){
			$rearrange_items = apply_filters('eventon_eventcard_boxes',array(
				...
				'evoet'=>array('evoet',__('Event Type Terms','eventon')),// @since 5.0
			));
	...
	// event top fields - @version 4.1
		function get_eventtop_all_fields(){
			...
			foreach( eventon_get_valid_ett() as $key=> $name){
				$base['eventtype'.($key==1? '': $key)] = __('Event Type' ,'eventon'). ($key==1? '': $key);
			}
		...
		function get_eventtop_fields_array(){
			...
			if(!$evo_etl){
				...
				// add event types	
				foreach( eventon_get_valid_ett() as $key=> $name){
					$evo_etl['c3'][] = array('f' => 'eventtype'.($key==1? '': $key), 'v'=> 'y' );					
				}		
	...
	// get event type #1 font awesome icon @updated 5.0
		function get_tax_icon($tax, $term_id, $opt, $termmeta= ''){

			if(!empty($opt['evcal_hide_filter_icons']) && $opt['evcal_hide_filter_icons']=='yes') return false;

			$icon_str = false;
			if($tax == 'event_type'){
				$termmeta = evo_get_term_meta( $tax, $term_id, $termmeta );
				if( !empty($termmeta['et_icon']) )	$icon_str = '<i class="fa '. $termmeta['et_icon']  .'"></i>';
			}
			return $icon_str;
		}

		// @since 5.0
		function get_tax_icon_var ($tax, $term_id , $termmeta = ''){
			$icon = '';
			if($tax == 'event_type'){
				$icon = evo_get_term_meta_field( $tax, $term_id, 'et_icon', $termmeta );
			}
			return $icon;
		}
		function get_tax_color( $tax, $term_id, $termmeta = ''){
			$color = '';
			if($tax == 'event_type'){
				$color = evo_get_term_meta_field( $tax, $term_id, 'et_color', $termmeta );
			}
			return $color;
		}
