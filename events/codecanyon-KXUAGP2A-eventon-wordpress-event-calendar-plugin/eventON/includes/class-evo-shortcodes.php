...
class EVO_Shortcodes {
	...
	// single events @updated 5.0
		public function single_event_box($atts){
			$atts = shortcode_atts([
				'id' => '', 
				'repeat_interval'=> '0',
				'ep_display_style' => '', 
			...
			// support dymanic post id
			if( $atts['id'] === 'pid' ){
				if( !isset($GLOBALS['post']->ID) ) return "<p class=''>". evo_lang('Missing Event ID') ."</p>";

				$atts['id'] = (int)$GLOBALS['post']->ID;
			...
			// user interaction for this event box
				$ev_uxval = isset($args['ev_uxval']) ? $args['ev_uxval']: 4; // default open as event page
				$external_url = '';
				if( $args['open_as_popup']=='yes' || $ev_uxval ==3){
					$ev_uxval = 3;
					$args['show_exp_evc'] = 'no';// override expended event card
				}elseif(  $ev_uxval =='X'){
					$ev_uxval = 'X';
				}elseif(  $ev_uxval =='2' && !empty($args['ext_url'])){// external link
					$ev_uxval = '2';
					$external_url = $args['ext_url'];
				}elseif(  $ev_uxval =='1' ){// slidedown
					$ev_uxval = 1;
				}
		...
		public function eventon_anywhere_event($atts){
			...
			// open event as single event page
				if( $ev_uxval == 4){
					...
					echo "<a href='".$EVENT->get_permalink() ."' class='eventon_anywhere event_{$id}_{$repeat_interval} {$extra_classes}'>{$cta_text}</a> ";
			...
			// open as ajax lightbox
				if( $ev_uxval == 3){
					...
					echo "<a data-sc='". json_encode( $args ) ."' href='".$EVENT->get_permalink() ."' class='eventon_anywhere evoajax event_{$id}_{$repeat_interval} {$extra_classes}'>{$cta_text}</a> ";
