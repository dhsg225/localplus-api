...
class evo_settings_settings{
		...
		return apply_filters('eventon_settings_tab1_arr_content', array(
			...
			array(
				...
				'fields'=>array(
					...
					array('id'=>'evo_mapfull',
						'type'=>'yesno',
						'name'=>__('Add button option to allow maps to be opened as full page','eventon'), 
						'legend'=>__('This will add a button over existing maps, allowing visitors to open map as full page lightbox and get directions.','eventon'),
						'ver'=> '5.0',
					),
					array('id'=>'evo_geoOSM',
						'type'=>'yesno',
			...
			array(
				...
				'fields'=> apply_filters('eventon_settings_time', array(
					...
					array('type'=>'sub_section_open','name'=>__('Front-end Time/Date Settings','eventon')),
						...
						array(
							'id'=>'evo_show_localtime','type'=>'yesno',
							'name'=>__('Enable "View in my time" button on frontend events - This will help visitor to localize event time','eventon'), 
							...
							'afterstatement'=>'evo_show_localtime'
						),
							array('id'=>'evo_show_localtime','type'=>'begin_afterstatement'),
							array(
							    'id' => 'evo_show_loct_on',
							    'type' => 'yesno',
							    'name' => __('Display Local Time on Load', 'eventon'),
							    'legend' => __('Automatically show event times in the visitor’s local timezone when the calendar loads, no clicks required.', 'eventon'),
							    'ver'=> '4.9',
							),
							array('id'=>'evo_show_localtime','type'=>'end_afterstatement'),
					...
					array('type'=>'sub_section_open','name'=>__('Add to Calendar Settings','eventon')),
						array('id'=>'evo_ics_format','type'=>'select',
							'options' => [
								'utc'=> __('UTC format'),
								'local'=> __('Local time format with TZID'),
							],
							'name'=>__('Add to calendar ICS file format','eventon'), 
							'tooltip'=>__('UTC format convert event time to UTC while local time pass event in event timezone with TZID','eventon'),
							'ver'=> '4.9.6'
						),
					array('type'=>'sub_section_close'),
			...
			array(
				...
				'fields'=>array(
					...
					array('id'=>'evo_filter_color',
						'type'=>'yesno',
						'name'=>__('Colorize event type filter dropdown','eventon'),
						'legend'=> __('Use event type category colors to colorize the event type 1 dropdown items list. Must set colors for each event type for it to colorize, otherwise will stay white.','eventon'),
						'ver'=> '5.0'
					),
					array('id'=>'evo_filter_hide_empty',
						'type'=>'yesno',
						'name'=>__('Hide empty event type & other taxonomy terms (with no events)','eventon'),
						'legend'=> __('This will hide all the event taxonomy filters that have no events','eventon'),
						'ver'=> '5.0'
					),
			)),
			array(
				...
				'fields'=>array(	
					...
					array('type'=>'sub_section_open','name'=>__('Other EventCard Settings','eventon')),
						...
						array(
							'id'=>'evo_ecard_lbnav',
							'type'=>'yesno',
							'name'=>__('Enable navigating between events on open event lightbox','eventon'),
							'legend'=>__('Add navigating between next and previous event in lightbox view, when events are opened as lightbox.','eventon'),
							'ver'=> '4.9', 'beta'=> true
						),				
						array(
							'id'=>'evo_morelass','type'=>'yesno',
							'name'=>__('Show full event description','eventon'),
							'legend'=>__('If you select this option, you will not see More/less button on EventCard event description.','eventon') 
						),
			...
			,array(
				'id'=>'evcal_010',
				'name'=>__('Event Categories, Taxonomies, & Multi Data Types','eventon'),
			...
			,array(
				...
				'fields'=>array(			
					...
					array('id'=>'evo_ditch_sin_template','type'=>'yesno',
						...
						array('id'=>'evcal__note','type'=>'note',
							'name'=> sprintf(__('<a href="%s" target="_blank"class="evo_trig_doc">Learn How to customize events archive page</a></br>' ,'eventon'), 
					...
					'fields'=> apply_filters('evo_settings_advanced', array(
						array('type'=>'sub_section_open','name'=>__('WP EventON Core Settings' ,'eventon')),					
							...
							array('id'=>'evo_cache_events',
								...
								'legend'=>__('This will cache the event data for calendar for upto 1 hour. Any new data create during this cache window will not appear on the calendar. This is best suited if same event data is loaded many times.','eventon')
							),
							array('id'=>'evo_restrict_csv',
								'type'=>'yesno',
								'name'=>__('Restrict Export Events as CSV','eventon'), 
								'legend'=>__('Restricting download all events as CSV will make sure only logged-in users can download the events data as CSV, otherwise everyone can download this.','eventon'),
								'ver'=>'4.9.11'
							),
						array('type'=>'sub_section_close'),
						...
						*/
						...
						array('type'=>'sub_section_open','name'=>__('Autonomous Functions' ,'eventon')),
							...
							array('id'=>'evcal_move_trash','type'=>'yesno','name'=>__('Auto move events to trash when the event date is past','eventon'), 'legend'=>__('This will move events to trash when the event end date is past current date. This action is performed daily via cron jobs. This will not be performed on repeat, month/year long events.','eventon'),
								'afterstatement'=> 'evcal_move_trash'
							),
								['type'=> 'begin_afterstatement', 'id'=> 'evcal_move_trash'],
								array(
									'id'=>'evcal_delete_past_events',
									'type'=>'yesno',
									'name'=>__('Delete past events completely (Can not reverse this)','eventon'), 
									'legend'=>__('Delete event posts, post meta values and taxonomy relationships completely from your website & database, for past event. This action is not reversable.','eventon')
								),
			...
			array(
				...
				'fields'=>array(	
					...
					array('type'=>'sub_section_open','name'=>__('Other Debug & Diagnose Settings','eventon')),
						...
						[
							'id'=> 'evo_fusion_off','type'=>'yesno',
							'name'=>__('Disable saving all Fusion/Avada related data into events','eventon'),
							'legend'=>__('Stop saving any event post meta values that have fusion or avada content.','eventon'),
							'ver'=> '4.9.10',
						],
					array('type'=>'sub_section_close'),
				)
	...
	// single events
		function single_events(){
			...
			$data[] = array(
				'id'=>'evo_note_1',
				'type'=>'notice',
				'name'=> sprintf(__('Note: <b>%s</b> will open event as lightbox on page load.','eventon'), home_url().'/events/one-event/{event_id}/{repeat_index}/'), 
				'legend'=>__('if you add these url changes, visiting that page will open the specific event when page loads. Refresh permalink if the url does not work.','eventon'),
				'ver'=> '4.9','beta'=> true
			);	
	...
	// HTML for eventcard designer
		function eventcard_designer_btn(){
			...
			EVO()->elements->get_element([
				...
				'name'=>__('EventCard Designer','eventon'),
				'description'=>__('Configure Event Card Structure and Layout','eventon'),
				'field_after_content'=> __("Configure",'eventon'). "<i class='fa fa-arrow-right evomarl15'></i>",
				'row_class'=> 'evo_bordern evomarb5',
				'field_class'=>'evo_eventcard_designer',
				'row_style'=> '    background-color: #e9ddff;',
				'row_class'=> 'evopad20i evobr10i',
				'trig_type'=> 'trig_lb',
				'trig_data'=> [
					'adata'=>[
						'a'=> 'eventon_load_ecard_designer',
					],	
					'lbdata'=> [
						'class'=> 'evo_ecard_designer',
						'title'=> __('EventCard Designer'),
	...
	// event type categories
		function event_type_categories(){
			...
			// taxonomy setting
			$etc[] =	array('type'=>'sub_section_open','name'=>__('Taxonomies Archive Page Settings' ,'eventon'));
			$etc[] = [
				'type'=> 'yesno','name'=>__('Hide upcoming events on Event Locations'), 'id'=> 'evo_tax_ue_event_location'
			];
			$etc[] = [
				'type'=> 'yesno','name'=>__('Hide upcoming events on Event Organizers'), 'id'=> 'evo_tax_ue_event_organizer'
			];

			$etc = apply_filters('evo_settings_etc_middle', $etc);
			$etc[] = ['type'=>'sub_section_close'];

			// for each Multi Data Types
			$etc[] = array('id'=>'evo_subheader','type'=>'subheader','name'=>__('EventCard Multi Data Types','eventon'));
			$etc[] = array('id'=>'evo_note','type'=>'note',
				'name'=>sprintf(__('This allow you to create multiple items of data for a type and select more than one of these data to show in eventCard. And they are accessible across all events. <a href="%s" class="evo_trig_doc evomarr10">%s</a> <a href="%s" class="evo_trig_doc">%s</a>'),
					'https://docs.myeventon.com/documentations/setup-use-multi-data-types/',
					__('How to set up Multi Data Types','eventon'),
					'https://docs.myeventon.com/documentations/various-creative-ways-to-use-multi-data-types/',
					'Various Ways to Use Multi Data Types'
				)
			);
			...
			for( $y=1; $y <= EVO()->mdt->evo_max_mdt_count(); $y++){		
				...
				// Additional fields
					for( $z=1; $z <= EVO()->mdt->evo_max_mdt_addfield_count(); $z++){
						$_field_pre_var = 'evcal_mdta_'.$y.'_'.$z;

						$name_additions = $z == 1 ? ' ['.__('Used in Sorting') .']':'';
						$etc[] = array('id'=>$_field_pre_var,'type'=>'yesno',
							'name'=>__('Enable Additional Data Field #'.$z,'eventon') . $name_additions,
							'legend'=> __('This will activate additional data field for this data type'),
							'afterstatement'=> $_field_pre_var
						);
						$etc[] = array('id'=>$_field_pre_var,'type'=>'begin_afterstatement');
						$etc[] = array('id'=>'evcal_mdta_name_'.$y.'_'.$z,'type'=>'text','name'=>__('Data Field Name','eventon'));
						$etc[] = array('id'=>$_field_pre_var .'_fv','type'=>'text','name'=>__('(optional) Pre-set Field Values (Separate by commas e.g., Gold, Silver)','eventon'));
						$etc[] =array('id'=>'evo_mdt_001','type'=>'customcode',
								'code'=>$this->mdt_add_field_settings($y, $z)
						);
						$etc[] = array('id'=>$_field_pre_var,'type'=>'end_afterstatement');
					}
		...
		private function mdt_add_field_settings($y, $z){
			$_field_pre_var = 'evcal_mdta_'.$y.'_'.$z;
			$out = '<div class="evodfx evofxdrr evogap10">';
			$out .= EVO()->elements->get_element([
				'id'=> $_field_pre_var.'_v',
				'type'=> 'select',
				'name'=> __('Visibility'),
				'options'=> [0 => 'Everywhere', 1 =>'Only Admin'],
				'value'=> EVO()->cal->get_prop( $_field_pre_var.'_v' )
			]);
			$out .= EVO()->elements->get_element([
				'id'=> $_field_pre_var.'_ft',
				'type'=> 'select',
				'name'=> __('Field Type'),
				'options'=> [0 => __('Text Field'), 1 => __('Pre-set Values')],
				'value'=> EVO()->cal->get_prop( $_field_pre_var.'_ft' )
			]);
			$out .= "</div>";
			return $out;
