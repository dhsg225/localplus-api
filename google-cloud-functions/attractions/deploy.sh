#!/bin/bash
set -e
echo "🚀 Deploying attractions Cloud Function..."
...
# Deploy the function
gcloud functions deploy localplus-api-attractions \
  --gen2 \
  --runtime=nodejs20 \
  --region=us-central1 \
  --source=. \
...
echo ""
echo "📋 Function URL:"
gcloud functions describe localplus-api-attractions --gen2 --region=us-central1 --format='value(serviceConfig.uri)'

echo ""
echo "🧪 Test with:"
echo "   curl \$(gcloud functions describe localplus-api-attractions --gen2 --region=us-central1 --format='value(serviceConfig.uri)')"
