// [2025-12-02] - Attractions API endpoint for Google Cloud Functions
  ...
  try {
    ...
    if (token) {
      try {
        ...
        console.warn('[Attractions GCF] Error setting auth session:', err.message);
      }
    }
    ...
    if (req.method === 'GET') {
      ...
      console.log('[Attractions GCF] Fetching attractions...');
      ...
      if (error) {
        console.error('[Attractions GCF] Error fetching attractions:', error);
      ...
      console.log(`[Attractions GCF] Found ${data?.length || 0} attractions.`);
    ...
    if (req.method === 'POST') {
      ...
      if (!user) {
        return res.status(401).json({ error: 'Unauthorized', message: 'User not authenticated for creating attractions.' });
      }
      ...
      if (error) {
        console.error('[Attractions GCF] Error creating attraction:', error);
        return res.status(500).json({
    ...
    console.error('[Attractions GCF] Unexpected error:', err);
    return res.status(500).json({
