#!/bin/bash
# Deployment script for auth Cloud Function
...
gcloud functions deploy localplus-api-auth \
  --gen2 \
  --runtime nodejs20 \
  --region us-central1 \
  --source . \
...
echo ""
echo "📋 Function URL:"
gcloud functions describe localplus-api-auth --gen2 --region us-central1 --format='value(serviceConfig.uri)' 2>/dev/null || echo "   Run: gcloud functions describe localplus-api-auth --gen2 --region us-central1 --format='value(serviceConfig.uri)'"
